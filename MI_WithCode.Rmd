---
title: 'Hydrobiologie-GBIF : Projet MacroInvertébrés 2018'
...
```{sql connection=db, max.print=21}
...
**Afin de ne pas encombrer la publication avec des données ayant une trop faible résolution taxonomique, uniquement les rangs espèce/genre sont gardés dans cette proposition. Cela peut être modifié à tout moment.** 
...
```
--select * from Taxons_tris;
alter table taxons_tris add column Publication text default NULL; -- Gardé ou non pour la publication
alter table Taxons_tris add column Justification text default NULL; -- Justification Si exclus
-- 1 Les Familles (à rejetter) ;
select * from Taxons_tris where trim(scientificname) like '%Dae'; -- 185
update Taxons_tris set Justification ='Famille' where trim(scientificname) like '%Dae';
-- bis, pour les taxons incluant une terminaison en 'Dae' mais pas sur le dernier mot du champ (attention particulière au screening de ce qui sera rejetté);
select * from Taxons_tris where trim(scientificname) like '%Dae%' and justification is null; -- "Chironomidae autres" comme seule entrée...
update Taxons_tris set Justification ='Famille' where trim(scientificname) like '%Dae%'  and justification is null;
-- Les noms au niveau de l'espèce ( à garder) ;
select * from Taxons_tris where trim(scientificname) like '% %' and justification is null;-- ceux constitué de deux mots, pas encore rejettés.
update Taxons_tris set Justification ='Espèce' where trim(scientificname)like '% %' and justification is null; -- 187 espèces a priori
-- Les noms au niveau du genre : difficile à isoler techniquement... les mots uniques ayant une terminaison compatible sont screenés un à uns
select * from Taxons_tris where trim(scientificName) not like '% %' and justification is null; -- 278 noms de genre potentiels, exclure les plus évidents mais une verrifcation sera nécéssaire(classes, ordres, invalides).
update Taxons_tris set Justification = "Genre" where trim(scientificName) not like '% %' and justification is null and scientificName not in ("Achetes","Amphipodes", "Annelides","Bivalves","Bryozoaires"
, "Cladoceres","Cnidaires" , "Coleoptera", "Coleopteres", "Copepodes", "Crustacea", "Decapoda", "Diptera", "Dipteres" , "Ephemeropteres", "Ephemeroptera","Exuvies", "Gasteropodes",  "Gasteropoda" , "Heteroptera", "Hydracariens",
"Hydrozoaires", "Hymenoptera" , "Insecta", "Isopodes","Lepidoptera","Megaloptera", "Mollusques", "Nemathelminthes",  "Nematodes","Nemertiens" ,"Odonata","Odonates", "Oligochetes", "Plathelminthes","Plecoptera", "Plecopteres", "Poissons",
"Polychetes","Spongiaires","Trichoptera",  "Trichopteres", "Triclades", "Turbellaries","Vertebrata", "Fourreaux de Trichoptera");
-- Supprimer encodages non valides (et champs vides)
update taxons_tris set Justification = 'Invalide' where scientificname in ("Aucun taxon", "", " " );
-- supprimer noms correspondant à un niveau taxo supérieur à la famille ;
update taxons_tris set Justification = 'HigherRank' where  scientificName in ("Achetes","Amphipodes", "Annelides","Bivalves","Bryozoaires"
, "Cladoceres","Cnidaires" , "Coleoptera", "Coleopteres", "Copepodes", "Crustacea", "Decapoda", "Diptera", "Dipteres" , "Ephemeropteres", "Ephemeroptera","Exuvies", "Gasteropodes",  "Gasteropoda" , "Heteroptera", "Hydracariens",
"Hydrozoaires", "Hymenoptera" , "Insecta", "Isopodes","Lepidoptera","Megaloptera", "Mollusques", "Nemathelminthes",  "Nematodes","Nemertiens" ,"Odonata","Odonates", "Oligochetes", "Plathelminthes","Plecoptera", "Plecopteres", "Poissons",
"Polychetes","Spongiaires","Trichoptera",  "Trichopteres", "Triclades", "Turbellaries","Vertebrata","Fourreaux de Trichoptera");
-- Etablir les choix (garder/rejet) en fonction des justifications ; 
update Taxons_tris set publication = 'N' where justification in ('Famille', 'Invalide', 'HigherRank');
update Taxons_tris set publication = 'Y' where justification in ('Espèce', 'Genre');
select * from Taxons_tris where publication is null;-- tout est attribué
-- tout est traité, reste à valider les espèces et genres par la taxonomie GBIF (surtout les taxons identifés comme des genres).

```


  + **Exemples de taxons rejettés de la publication (249 entrées):**
...
```{sql connection=db, max.print= 20}
select * from Taxons_tris where publication = 'Y'; -- 403 genres et espèces... travail à valider (GBIf, fRC?)
```
...
Il reste donc 53697 occurrences de 403 taxons (sp./gen) sur les 197.000 enregistrements/652 taxons (encodage distincts) que la collection comptait au départ.
...
L'information est en français dans la BD AQUABIO (GEN_D_TYPOLO.LIBELL). Les traductions suivantes sont proposées afin d'avoir un champ habitat en anglais. Il reste possible de simplement ne pas partager cette information.

*Traductions proposées pour les types de milieux :*

```{sql connection=db}
select * from habitat_dico ;
```

```{sql connection=db}
alter table occurrences add column Habitat text default null;
```
```{sql connection=db}
update occurrences set habitat =(select GBIF_term from habitat_dico where habitat=occurrences.TYPO_COURS_DO)
```

*Combien de données pour chaque milieu? :*

```{sql connection=db}
select distinct habitat, count(*) from occurrences group by habitat;
```
...
### Localisation -- *DecimalLongitude, DecimalLatitude, GeodeticDatum, VerbatimCoordinates VerbatimCoordinatesSystem, Locality, Municipality* 
...
Les champs 'Locality, Municipality sont quant à eux déduis depuis les colonnes respectives 'GEN_D_LOCALI.LIBELL' et 'GEN_D_COMMUN.LIBELL' ;

```{sql connection=db}
alter table occurrences add column Locality text default null ;
```
```{sql connection=db}
alter table occurrences add column Municipality text default null ;
```
```{sql connection=db}
update occurrences set locality = localite;
```
```{sql connection=db}
update occurrences set municipality = municipalite;
```
...
```
  ...
  + **la colonne DarwinCore n'accepte que des valeurs purement numériques. Afin d'ajouter ce champ à la publication, les conversions suivantes sont proposées pour adapter les valeurs de la colonne "GEN_D_PRECXY.LIBELL" :**

```{sql connection=db}
select * from conversions_incertitude;
```

```{sql connection=db}
update Occurrences set CoordinateUncertaintyInMeters = '100'
where prec = '<100';
```
```{sql connection=db}
...
select * from occurrences where n=0 or n='' ;
```
...
**"DEMNA : occurrences of macroinvertebrates in monitored surface water bodies of Wallonia"**


```{sql connection=db}
alter table occurrences add column DatasetName text default "DEMNA : occurrences of macroinvertebrates in monitored surface water bodies of Wallonia";
```
...
IdentifiedBy, EstablishmentMeans, EventDate, Year, Month, Day, VerbatimEventDate,Habitat, Municipality, Locality, DecimalLongitude, DecimalLatitude, CoordinateUncertaintyInMeters, GeodeticDatum,VerbatimCoordinates, VerbatimCoordinateSystem from occurrences;  
```
...
## 4.2. Proposition ;
  ...
  + **Basic Metadata :**
    + Title :  DEMNA : Occurrences of macroinvertebrates in monitored surface water bodies of Wallonia
...
```
  "DEMNA : Occurrences of macroinvertebrates in monitored surface water bodies of Wallonia" is an occurrence dataset created by the Hydrobiology unit of the Département de l'Etude du Milieu Naturel et Agricole (DEMNA-SPW). The data come from an ongoing network for the monitoring of the biological quality of water bodies in Wallonia, set up to undertake the EU Water framework directive requirements (since 2004). The Current monitoring network is composed of more than 440 sites, distributed among all streams (brooks to largest rivers/canals) from the hydrographic districts of the Meuse, the Scheldt, the Rhine and the Seine (Wallonia areas). Macroinvertebrates are one of the 4 indicator groups used to assess the biological quality of water bodies with macrophytes, diatoms, and fishes. The macroinvertebrate occurrences are derived from standardized IBGN/IBGA samplings, done according a methodology derived from T90-350 (2004) French standard (see Vanden Bossche & Usseglio-Polatera, 2005 in Hydrobiologia, 551: 253-271).Some IBGN/IBGA target taxa have been removed from the dataset, due to the low taxonomic resolution used (families,classes, orders). Some data can be previous to the set up of the current monitoring network, collected during other activies of the hydrobiology unit.More information is available upon request adressed to the DEMNA, and please note that this dataset does not replace any official convention on the provision of biological data delivered by the department.
```  
...
```
Monitoring the biological quality of surface waterbodies in Wallonia for the EU water framework directive. MacroInvertebrates are used to determine IBGN / IBGA indicators, that are reported to Europe annually.
```
...
```
15 January 1990 - 29 September 2016
```
...
```
  ...
  + **Taxonomic scope :**
```{sql connection=db}
select count(distinct(scientificName)) from occurrences ;
```
```{sql connection=db}
select count(distinct(scientificname)) species_genres from Taxons_tris where justification like 'Esp%' union select count(distinct(scientificname)) genus from Taxons_tris where justification like 'gen%' ; -- 183 espèces, 228 genres.
```
  
```
Freshwater macroinvertebrates from Wallonia. The observations include 403 taxa divided between species (178)and genus (225) in the last version. The dataset does not match exactly what is expected for the IBGN/IBGA protocols, and has been adapted to be more scientifically relevant : family-level items (and higher) has been removed and some had hoc observations made by the laboratory staff might be included.
```
...
![](C:/Users/46123/Pictures/logo_dgo3_0.png)   
